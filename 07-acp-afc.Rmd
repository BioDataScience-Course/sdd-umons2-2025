...
-   **Étape 2 :** test de Chi^2^ d'indépendance, voir `r course_link("section correspondante du cours de SDD I", course = 1, "chi2", "test-chi2-dind%C3%A9pendance")`. L'hypothèse nulle du test est que les deux variables sont indépendantes l'une de l'autre. L'hypothèse alternative est qu'une dépendance existe. Si la réponse aux questions est indépendante du niveau d'éducation (pas de rejet de H~0~), notre analyse est terminée. Sinon, il faut approfondir... Choisissons notre seuil $\alpha$ à 5% avant de réaliser notre test.
