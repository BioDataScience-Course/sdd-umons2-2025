...
-   Être capable d'ordiner des variables qualitatives sous forme de tableaux cas par variables ou de tables de contingences à double entrée à l'aide de l'**Analyse Factorielle des Correspondances** (AFC).
...
## Réduction de dimensions grâce à l'ordination {#reductiondimensions}
...
`r h5p(104, "B07Ha_acp", height = 270, toc = "Principe de l'ACP")`

### Indiens diabétiques

```{r acppima, echo=FALSE}
#'
...
`r h5p(106, "B07Hc_scree", height = 270, toc = "Analyse du graphique des éboulis")`
...
Le graphique entre PC2 et PC3 complète l'analyse, mais n'apportant rien de plus, il peut être typiquement éliminé de votre rapport si vous arrivez à la même conclusion avec vos propres données.
...
`r h5p(107, "B07Hd_loadings", height = 270, toc = "Représentation dans l'espace des variables")`
...
Nous avons 12 variables quantitatives continues, 1 variable encodée comme numérique, mais qualitative en réalité (`maturity`) et une variable qualitative (`origin`). Notez la distribution très asymétrique et similaire (voir colonne `hist`) de toutes les variables quantitatives. Les variables `origin` et `maturity` ne pourront pas être utilisées, mais seront éventuellement utiles pour colorer les points dans nos graphiques. Qu'en est-il des corrélations entre les 12 variables quantitative ?
...
```{r echo=FALSE, fig.width=9, fig.height=5, message=FALSE, purl=FALSE}
dat <- dtx(Station = 1:9,
...
```{r echo=FALSE, fig.width=9, fig.height=5, purl=FALSE}
...
```{r echo=FALSE, fig.width=4.5, fig.height=5, purl=FALSE}
plt <- ggplot(dat) +
  #geom_point(mapping = aes(x = Var1, y = Var2, label = Station)) +
  theme_bw() +
  theme(plot.background = element_blank()) +
...
```{r echo=FALSE, fig.width=4.5, fig.height=1, purl=FALSE}
plt2 <- ggplot(dat) +
  theme_bw() +
  theme(plot.background = element_blank()) +
...
```{r, fig.width=4.5, fig.height=4.5, echo=FALSE, message=FALSE, warning=FALSE, purl=FALSE}
# Point
...
-   Étape 1 : centrage des données (soustraction de la moyenne de chaque colonne, ce qui donne une moyenne nulle pour chaque colonne des données centrées)
...
```{r assign_B07Ia_acp_afc_I, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B07Ia_acp_afc", part = "I",
    url = "https://github.com/BioDataScience-Course/B07Ia_acp_afc",
    course.ids = c(
      'S-BIOG-061' = !"B07Ia-{YY}M-acp-afc"),
    course.urls = c(
      'S-BIOG-061' = !"{assign_url$B07Ia_acp_afc}"),
    course.starts = c(
      'S-BIOG-061' = !"{class1_start(mod, 'B07')}"),
    course.ends = c(
      'S-BIOG-061' = !"{n3_end(mod, 'B07')}"),
    term = "Q2", level = 3,
    toc = "ACP et AFC en pratique (partie I)")
```
...
`r h5p(108, "B07He_afc", height = 270, toc = "Rôle de l'AFC")`
...
-   **Étape 2 :** test de Chi^2^ d'indépendance, voir `r course_link("section correspondante du cours de SDD I", course = 1, "chi2", "chi2independance")`. L'hypothèse nulle du test est que les deux variables sont indépendantes l'une de l'autre. L'hypothèse alternative est qu'une dépendance existe. Si la réponse aux questions est indépendante du niveau d'éducation (pas de rejet de H~0~), notre analyse est terminée. Sinon, il faut approfondir... Choisissons notre seuil $\alpha$ à 5% avant de réaliser notre test. 
...
`r h5p(109, "B07Hf_afc2", height = 270, toc = "Chi2 et l'AFC")`
...
```{r assign_B07Ia_acp_afc_II, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B07Ia_acp_afc", part = "II",
    ...
    course.ends = c(
      'S-BIOG-061' = !"{n3_end(mod, 'B07')}"),
    term = "Q2", level = 3,
    toc = "ACP et AFC en pratique (partie II)")
```
...
```{r assign_B07Ca_multi, echo=FALSE, results='asis', purl=FALSE}
if (exists("challenge"))
  challenge("B07Ca_multi", part = NULL,
    url = "https://github.com/BioDataScience-Course/B07Ca_multi",
    course.ids = c(
      'S-BIOG-061' = !"B07Ca-{YY}M-multi"),
    course.urls = c(
      'S-BIOG-061' = !"{assign_url$B07Ca_multi}"),
    course.starts = c(
      'S-BIOG-061' =  !"{mod['B07', 'challenge']} 09:45:00"),
    course.ends = c(
      'S-BIOG-061' = !"{mod['B07', 'challenge']} 12:30:00"),
    term = "Q2",
...
**Continuez votre projet de groupe entamé au module 6.**

```{r assign_B06Ga_open_data_II, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment2"))
  assignment2("B06Ga_open_data", part = "II",
...
## Récapitulatif des exercices {#recapitulatifb07}
...
-  [Document HTML](more/07-acp-afc.html)
-  [Script R](https://github.com/BioDataScience-Course/sdd-umons2-2025/blob/main/R/07-acp-afc.R){target="top"}
