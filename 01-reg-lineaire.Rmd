...
Outre les outils logiciels, ce cours s'appuie sur des notions que nous avons déjà abordées dans le premier cours comme la réalisation de `r course_link("graphiques en nuage de points", course = 1, "nuagedepoints")` et de `r course_link("graphiques qunatile-quantile", course = 1, "graphique-quantile-quantile")`, le `r course_link("test de Student", course = 1, "test-t-de-student")`, l'`r course_link("analyse de la varaince à un facteur", course = 1, "anova-%25C3%25A0-un-facteur")` et `r course_link("à deux facteurs", course = 1, "anova-%25C3%25A0-deux-facteurs")` ou encore les `r course_link("associations entre les variables", course = 1, "corr%25C3%25A9lation")`. **Assurez-vous de bien maîtriser ces différentes parties du cours 1 avant d'entamer le présent module.**
...
-   La **corrélation** quantifie l'allongement dans une direction préférentielle du nuage de points à l'aide des coefficients de corrélation linéaire de **Pearson** ou non linéaire de **Spearman**. Ce niveau d'association a été traité dans le `r course_link("module 6 du cours 1", course = 1, "corr%25C3%25A9lation")`. Il est purement descriptif et n'implique aucune autre hypothèse sur les données observées.
...
Rappelons-nous que dans le `r course_link("module 6 du cours de science des données 1", course = 1, "corr%25C3%25A9lation")`, nous avons étudié l'association de deux variables quantitatives (ou numériques). Nous utilisons donc une matrice de corrélation afin de mettre en évidence la corrélation entre nos trois variables qui composent le jeu de donnée `trees`.
...
Cependant, n'oubliez pas qu'il est indispensable de visualiser le nuage de points pour ne pas tomber dans le piège mis en avant par le `r course_link("jeu de données artificiel appelé 'quartet d'Anscombe'", course = 1, "corr%25C3%25A9lation", "importance-des-graphiques")` qui montre très bien comment des données très différentes peuvent avoir une même moyenne, une même variance et un même coefficient de corrélation. Un graphique de type matrice de nuages de points est tout indiqué ici.
