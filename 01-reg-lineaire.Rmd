...
Avant de vous lancer tête baissée dans de la matière nouvelle, assurez-vous d'avoir installé la SciViews Box `r !'{svbox}'`. Si ce n'est pas encore fait, retournez à la [page d'accueil pour bien débuter](https://wp.sciviews.org).

RStudio et (R) Markdown/Quarto ne doivent plus avoir de secrets pour vous. Les modifications par rapport à l'année précédente vous sont présentées à la première séance du cours. La présentation est disponible `r !"[ici](https://github.com/BioDataScience-Course/sdd_lessons/raw/{acad_year}/B00/presentations/B00Pa_intro.pdf)"` .

##### À vous de jouer ! {.unnumbered}

`r h5p(190, "B01Aa_tools", height = 270, toc = NULL)`
...
`r h5p(191, "B01Ab_git_summary", height = 270, toc = NULL)`

L'ensemble de ces outils a été abordé dans le premier module et dans les annexes du cours précédents `r course_link("SDD I", course = 1, "outils")`.

Outre les outils logiciels, ce cours s'appuie sur des notions que nous avons déjà abordées dans le premier cours comme la réalisation de `r course_link("graphiques en nuage de points", course = 1, "nuagedepoints")` et de `r course_link("graphiques quantile-quantile", course = 1, "quantilequantile")`, le `r course_link("test de Student", course = 1, "studentt")`, l'`r course_link("analyse de la variance à un facteur", course = 1, "anovaunfacteur")` et `r course_link("à deux facteurs", course = 1, "anovadeuxfacteurs")` ou encore les `r course_link("associations entre les variables", course = 1, "correlation")`. **Assurez-vous de bien maîtriser ces différentes parties du cours 1 avant d'entamer le présent module.**

##### À vous de jouer ! {.unnumbered}

`r h5p(192, "B01Ac_notions_sdd1", height = 270, toc = NULL)`
...
`r h5p(193, "B01Ha_variables_parametres", height = 270, toc = "Paramètres et variables d'une équation")`
...
-   La **corrélation** quantifie l'allongement dans une direction préférentielle du nuage de points à l'aide des coefficients de corrélation linéaire de **Pearson** ou non linéaire de **Spearman**. Ce niveau d'association a été traité dans le `r course_link("module 6 du cours 1", course = 1, "correlation")`. Il est purement descriptif et n'implique aucune autre hypothèse sur les données observées.
...
`r h5p(194, "B01Hb_association", height = 270, toc = "Association entre deux variables")`
...
## Régression linéaire simple {#reglinsimple}
...
Rappelons-nous que dans le `r course_link("module 6 du cours de science des données 1", course = 1, "correlation")`, nous avons étudié l'association de deux variables quantitatives (ou numériques). Nous utilisons donc une matrice de corrélation afin de mettre en évidence la corrélation entre nos trois variables qui composent le jeu de donnée `trees`.
...
`r h5p(195, "B01Hc_correlation", height = 270, toc = "Les coefficients de corrélation")`

Cependant, n'oubliez pas qu'il est indispensable de visualiser le nuage de points pour ne pas tomber dans le piège mis en avant par le `r course_link("jeu de données artificiel appelé 'quartet d'Anscombe'", course = 1, "correlation", "importance-des-graphiques")` qui montre très bien comment des données très différentes peuvent avoir une même moyenne, une même variance et un même coefficient de corrélation. Un graphique de type matrice de nuages de points est tout indiqué ici.
...
`r h5p(196, "B01Hd_somme_residus", height = 270, toc = "Qualité d'ajustement d'un modèle")`
...
p <- chart(data = models1, slope ~ intercept %col=%  fobj) +
  geom_point() +
  geom_point(data = sfilter(models1, data.table::frank(fobj) <= 10),
...
```{r, purl=FALSE}
best_models <- sfilter(models1, data.table::frank(fobj) <= 10)
```
...
`r h5p(197, "B01He_equation", height = 270, toc = "Écriture de l'équation du modèle")`
...
`r h5p(198, "B01Hf_lm", height = 270, toc = "La fonction lm()")`

### Résumé avec `summary_()`

Avec `summary_()` nous obtenons un résumé condensé des informations les plus utiles pour chaque objet dans R. notez que R de base utilise la fonction `summary()` (sans `_`). La différence entre les deux est que `summary_()` conserve aussi l'information sur l'objet qui est résumé, contrairement à `summary()`. Dans le cas de l'objet **lm**, l'information renvoyée est très utile pour interpréter notre régression linéaire.
...
*Pour l'instant, nous nous contenterons d'interpréter et d'utiliser les informations issues de `summary_()` dans sa partie supérieure. Le contenu des trois dernières lignes sera détaillé dans le module suivant.*

Comme la sortie de `summary_()` pour un objet **lm**, bien que très utile, fait brut de décoffrage dans un rapport, vous pouvez utiliser ici aussi `tabularise()` pour mettre cela en forme :
...
`r h5p(199, "B01Hg_std_error", height = 270, toc = "Analyse du résumé de l'objet lm.")`
...
`r h5p(200, "B01Hh_resid", height = 270, toc = "Distribution homogène des résidus")`
...
```{block, type='warning', purl=FALSE}
**Attention !** Le graphique quantile-quantile a été abordé au premier cours de Science des Données Biologiques, mais il apparaît que de nombreux étudiants ne comprennent pas bien ce qu'il représente, ni ne savent comment l'utiliser dans le présent contexte.
```

Assurez-vous d'avoir bien compris le mécanisme à la base du graphique quantile-quantile et retournez voir `r course_link("l'explication dans le cours précédent", course = 1, "quantilequantile")` si nécessaire.
...
```{r assign_B01Ia_debug, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B01Ia_debug", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-015'         = !"{assign_url$B01Ia_debug}"),
    course.starts = c(
      'S-BIOG-015'         = !"{class1_start(mod, 'B01')}"),
    course.ends = c(
      'S-BIOG-015'         = !"{n3_end(mod, 'B01')}"),
    term = "Q1", level = 3,
...
```{r, purl=FALSE}
# Graphique avec enveloppe de confiance à 95%
trees_lm %>.%
...
`r h5p(201, "B01Hi_tidy", height = 270, toc = "Les fonctions tidy() et glance()")`

##### À vous de jouer ! {.unnumbered}

```{r assign_B01Ib_abalone, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B01Ib_abalone", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-015'         = !"{assign_url$B01Ib_abalone}"),
    course.starts = c(
      'S-BIOG-015'         = !"{class1_start(mod, 'B01')}"),
    course.ends = c(
      'S-BIOG-015'         = !"{n3_end(mod, 'B01')}"),
...
## Récapitulatif des exercices {#recapitulatifb01}
...
-  [Document HTML](more/01-reg-lineaire.html)
-  [Script R](https://github.com/BioDataScience-Course/sdd-umons2-2025/blob/main/R/01-reg-lineaire.R){target="top"}
