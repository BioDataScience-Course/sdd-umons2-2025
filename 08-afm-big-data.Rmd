...
-   Pouvoir choisir les meilleurs outils de traitement et de remaniement de données en fonction de la taille des jeux de données à traiter, en particulier pour les (très) gros tableaux (= big data)
...
## Analyse factorielle multiple (AFM) {#afm}
...
Il contient trois groupes de variables distincts :
...
`r h5p(120, "B08Ha_formula", height = 270, toc = "Formule de l'AFM")`
...
```{r assign_B08Ia_mfa, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B08Ia_mfa", part = NULL,
    url = "https://github.com/BioDataScience-Course/B08Ia_mfa",
    course.ids = c(
      'S-BIOG-061' = !"B08Ia-{YY}M-mfa"),
    course.urls = c(
      'S-BIOG-061' = !"{assign_url$B08Ia_mfa}"),
    course.starts = c(
      'S-BIOG-061' = !"{class1_start(mod, 'B08')}"),
    course.ends = c(
      'S-BIOG-061' = !"{n3_end(mod, 'B08')}"),
    term = "Q2", level = 3,
    toc = "AFM sur oiseaux alpins")
```

```{r assign_B08Ib_zooscannet_I, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B08Ib_zooscannet", part = "I",
    url = "https://github.com/BioDataScience-Course/B08Ib_zooscannet",
    course.ids = c(
      'S-BIOG-061' = !"B08Ia-{YY}M-zooscannet"), # !!! Wrong name!
    course.urls = c(
      'S-BIOG-061' = !"{assign_url$B08Ib_zooscannet}"),
    course.starts = c(
      'S-BIOG-061' = !"{class2_start(mod, 'B08')}"),
    course.ends = c(
      'S-BIOG-061' = !"{n3_end(mod, 'B08')}"),
    term = "Q2", level = 3,
    toc = "AFM sur plancton (gros jeu de données) partie I")
```

## Big data {#bigdata}

```{r bigdata, echo=FALSE}
#'
...
Au départ, vos données sont stockées sur le disque, proviennent d'Internet, ou d'un package R. Dans `SciViews::R`, vous les lisez en mémoire vive avec la fonction `read()`. Cette fonction importe les données depuis différents formats. Nous allons considérer principalement deux formats très fréquents en biologie : CSV et Excel. Microsoft Excel a une limitation maximale de la taille du tableau à un peu plus d'un million de lignes et un peu plus de 16.000 colonnes. Donc, est-ce qu'un million de lignes, c'est à considérer comme du "big data" avec R aussi ? Pour y répondre, il faut déterminer si un tel tableau de données peut tenir en mémoire vive. Admettons que le tableau ne contienne que des nombres réels. Dans R, ces nombres sont représentés par des **"doubles"** qui occupent chacun 8 octets[^08-afm-big-data-4] (les entiers de R, dits **"long integers"** occupent, eux, 4 octets chacun). Si vous avez 10 variables matérialisées par des "doubles" dans ce tableau d'un million de lignes, vous aurez : 1.000.000 \* 10 \* 8 octets = 80 millions d'octets ou encore environ 80 mégaoctets. La mémoire vive d'un PC moyen fait environ 8 gigaoctets ou 8 Go, soit environ 8000 mégaoctets. Aucun problème pour R avec un tableau de cette taille ou même plus gros. **En matière de nombre de lignes, Excel déclare forfait vers le million, mais R peut facilement gérer des tableaux bien plus volumineux.**

[^08-afm-big-data-4]: Un octet est l'unité de base en informatique. Il est constitué de 8 bits. Le bit est l'unité minimale constitué de 1 ou 0, et stockant donc une information binaire. Un rassemblement de 8 bits en un octet permet de stocker 2^8^, ou encore 256 valeurs différentes. Pour plus de valeurs différentes, on rassemble plusieurs octets, donc 4 octets pour les entiers et 8 octets pour les valeurs numériques de type "double" en R.
...
```{r, purl=FALSE}
# Plus grand entier que R peut représenter
.Machine$integer.max
```
...
Si vous voulez essayer dans votre SciViews Box sur SaturnCloud ce qui suit, le package {babynames} n'étant pas installé par défaut, il vous faut l'installer d'abord comme ceci :
...
Ensuite, nous minutons le temps nécessaire au chargement du jeu de données `babynames`.

```{r, purl=FALSE}
# Chargement du dialecte SciViews::R
SciViews::R
# Lecture du jeu de données babynames + minutage
(time <- system.time(
  babynames <- read("babynames", package = "babynames")
)[["elapsed"]])
```

Il nous a fallu seulement `r round(time, 3)` secondes pour charger ce jeu de données en mémoire depuis la machine qui a compilé ce cours en ligne. Pour visualiser de manière compacte la structure du tableau, vous pouvez utiliser `str()` :
...
```{r, purl=FALSE}
# Taille du jeu de données babynames en mémoire
lobstr::obj_size(babynames)
```

Notre tableau occupe près de 75 millions d'octets (*Bytes* en anglais, d'où l'unité `B`). Pour connaitre la quantité de mémoire vive totale occupée par R, les packages et les objets chargés en mémoire, regarder dans l'onglet **Environment** de RStudio. Dans la barre d'outils, il y a un petit graphique en parts de tarte qui l'indique avec la valeur en `MiB` ou `GiB`. Les `MiB`, contrairement aux `MB` correspondent, eux, à 1024*1024 = 1.048.576 d'octets, donc un tout petit peu plus du 1.000.000. Confus ? Oui, ce n'est pas facile, il y a deux façons de calculer la taille en mémoire et sur le disque, avec le "kilo" octets qui vaut soit 1000 (`Ko`), soit 1024 (`Kio`). Soyez bien attentifs. Par exemple, la taille des disques durs et disques SSD sont généralement renseignés en `GB` en anglais (ou `Go` en français), des millions d'octets ou en `TB`/`To`, des "teraoctets", soit des milliards d'octets. Mais la taille de la mémoire vive est parfois renseignée en `GiB`, soit 1024 x 1024 x 1024 octets. Cela explique que vos partitions dépassent de peu les 900 Gio sur votre disque pourtant bien renseigné comme ayant une capacité de ... 1 téraoctet, ou encore 1.000 Go !
...
```{r, purl=FALSE}
# Détermination des prénoms les plus fréquents et minutage de l'opération
(time <- system.time(
  babynames %>.%
    sgroup_by(., name) %>.%
...
```{r, purl=FALSE}
# Initiation d'un data frame avec les minutages
timings <- dtx(
  format = c("csv", "csv.gz", "csv.bz2", "csv.xz"),
  ecriture_s = NA, lecture_s = NA, taille_Mo = NA)
```

Nous allons maintenant enregistrer notre tableau de données en CSV non compressé et le relire, tout en minutant les opérations d'écriture et de relecture. Le chemin d'accès au fichier est construit à l'aide de `path()`. Notez aussi que nous utilisons `csv_alt` qui utilise la fonction `readr::read_csv()` en interne car la fonction par défaut ne gère pas bien le format de compression BZ2.

```{r, purl=FALSE}
# Minutage de l'écriture du fichier CSV avec babynames
csv_file <- path(testdir, "babynames.csv")
(timings$ecriture_s[1] <- system.time(
  write$csv_alt(babynames, csv_file)
)[["elapsed"]])
```
...
```{r, purl=FALSE}
# Minutage de la lecture du fichier CSV avec babynames
(timings$lecture_s[1] <- system.time(
  babynames2 <- read$csv_alt(csv_file)
)[["elapsed"]])
```
...
```{r, purl=FALSE}
# Taille du fichier CSV babynames
...
```{r, purl=FALSE}
# Minutage écriture, lecture de babynames.csv.gz + taille du fichier
csv_gz_file <- path(testdir, "babynames.csv.gz")
(timings$ecriture_s[2] <- system.time(
  write$csv.gz_alt(babynames, csv_gz_file)
)[["elapsed"]])
(timings$lecture_s[2] <- system.time(
  babynames2 <- read$csv.gz_alt(csv_gz_file)
)[["elapsed"]])
...
```{r, purl=FALSE}
# Minutage écriture, lecture de babynames.csv.bz2 + taille du fichier
csv_bz2_file <- path(testdir, "babynames.csv.bz2")
(timings$ecriture_s[3] <- system.time(
  write$csv.bz2_alt(babynames, csv_bz2_file)
)[["elapsed"]])
(timings$lecture_s[3] <- system.time(
  babynames2 <- read$csv.bz2_alt(csv_bz2_file)
)[["elapsed"]])
...
```{r, purl=FALSE}
# Minutage écriture, lecture de babynames.csv.xz + taille du fichier
csv_xz_file <- path(testdir, "babynames.csv.xz")
(timings$ecriture_s[4] <- system.time(
  write$csv.xz_alt(babynames, csv_xz_file)
)[["elapsed"]])
(timings$lecture_s[4] <- system.time(
  babynames2 <- read$csv.xz_alt(csv_xz_file)
)[["elapsed"]])
...
# Tableau comparatif écriture, lecture et taille pour les différents formats
tabularise(timings)
```
...
```{r, purl=FALSE}
# Nettoyage de l'environnement R et des fichiers créés sur le disque
...
`r h5p(224, "B08Hc_compress", height = 270, toc = "Niveaux de compression")`
...
**Première option :** envisagez de stocker vos données (compression csv.xz si possible) ailleurs où elles sont accessibles via un lien URL. Pour cela, il vous faut un serveur de fichiers. Des stockages cloud comme OneDrive, Google Drive ... ne permettent en général pas le téléchargement direct de fichiers via une URL (en tous cas pas sans ruser) : il faut obligatoirement passer par l'interface du système. Par contre, certains dépôts spécialisés dans les données comme [Zeonodo](https://zenodo.org) vous permettent de créer un compte gratuit et d'ensuite partager des données, y compris de très, très gros fichiers (jusqu'à 50Go, voire plus avec autorisation à demander aux gestionnaires du site). Vous aurez en plus un DOI, c'est-à-dire, un identifiant unique pour faciliter l'accès (voir module 10). Dans cette première option, vous téléchargez le fichier depuis cette URL **à chaque fois que vous en avez besoin**. C'est une solution simple pour être certain d'avoir toujours la dernière version du fichier (dans le cas où il pourrait change). Vous utilisez juste `read()` avec l'URL.

**Deuxième option :** vous pouvez aussi utiliser un **cache** qui vous permettra de ne lire le fichier qu'une seule fois et de le stocker ensuite sur le disque. La fonction `read()` admet un argument `cache_file =` qui indique où stocker une copie du fichier. Le but est de le stocker dans le dépôt, mais dans un dossier qui sera ignoré de git, par exemple dans `data/cache`. Pour éviter que ce dossier ne soit repris dans le système de gestion de version, il faut aussi ajouter `data/cache/` écrit tel quel dans le fichier `.gitignore` à la racine du dépôt. La barre oblique à la fin est importante pour spécifier qu'il s'agit d'un dossier dont on veut ignorer tout le contenu. À partir du moment où vous aurez commité et pushé le `.gitignore` ainsi modifié, le fichier en cache ne sera pas repris dans les commits/pulls/ pushes. Cela donne quelque chose comme ceci (spécifiez toujours bien le format avec `$…` si vous chargez vos données depuis une URL) :
...
Un autre piège du cache est qu'il fige la version du fichier à la version lue au premier `read()`. Si vous voulez le mettre à jour plus tard, au cas où il existerait une version plus récente, vous devez pensez à effacer le fichier cache avant de faire un nouveau `read()`. Ceci forcera alors un nouveau téléchargement depuis l'URL.

##### À vous de jouer ! {.unnumbered}

`r h5p(225, "B08Hd_file_size", height = 270, toc = "Taille maximale de fichiers dans git/GitHub")`
...
`r h5p(226, "B08He_sql", height = 270, toc = "Bases de données et requêtes")`
...
Si jamais vous décidez d'imprimez le contenu de `db_babynames` tout entier, vous pouvez avoir l'impression qu'il contient les données, mais il n'en est rien. Une requête sur les 10 premières lignes du tableau est faite en interne dans la base de données. Ensuite ces 10 lignes sont affichées, et le résultat est jeté. Aucune donnée n'est réellement stockée dans `db_babynames`. D'ailleurs, ce n'est pas un data frame, mais une liste (voyez dans l'onglet **Environment**).
...
```{r assign_B08Ib_zooscannet_II, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B08Ib_zooscannet", part = "II",
    url = "https://github.com/BioDataScience-Course/B08Ib_zooscannet",
    course.ids = c(
      'S-BIOG-061' = !"B08Ia-{YY}M-zooscannet"), # !!! wrong name!
    course.urls = c(
    ...
    course.ends = c(
      'S-BIOG-061' = !"{n3_end(mod, 'B08')}"),
    term = "Q2", level = 3,
    toc = "AFM sur plancton (gros jeu de données) partie II")
```

**Continuez à travailler sur votre projet de groupe.**

```{r assign_B06Ga_open_data_III, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment2"))
  assignment2("B06Ga_open_data", part = "III",
    ...
    term = "Q2", level = 4, n = 4,
    toc = "Étude de données ouvertes choisies librement (III)")
```

## Récapitulatif des exercices {#recapitulatifb08}
...
-  [Document HTML](more/08-afm-big-data.html)
-  [Script R](https://github.com/BioDataScience-Course/sdd-umons2-2025/blob/main/R/08-afm-big-data.R){target="top"}
