...
## Analyse de données {#analysedonnees}
...
`r h5p(75, "B06Ha_stat_explo", height = 270, toc = "Objectif des statistiques exploratoires")`

## Distance entre individus {#distance}

```{r purldistance, echo=FALSE}
#'
...
`r h5p(76, "B06Hb_matrice", height = 270, toc = "Objectif de la matrice de distance")`
...
`r h5p(77, "B06Hc_euclidienne", height = 270, toc = "Utilisation de la distance euclidienne")`
...
`r h5p(78, "B06Hd_indice", height = 270, toc = "Choix des indices")`
...
## Regroupement avec CAH {#cah}

```{r purlcah, echo=FALSE}
#'
...
`r h5p(79, "B06He_methode", height = 270, toc = "Méthodes divisives ou agglomératives")`
...
`r h5p(80, "B06Hf_sequence", height = 350, toc = "Ordre des étapes pour obtenir un dendrogramme")`
...
```{r assign_B06Ia_fish_market_I, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B06Ia_fish_market", part = "I",
    url = "https://github.com/BioDataScience-Course/B06Ia_fish_market",
    course.ids = c(
      'S-BIOG-061' = !"B06Ia-{YY}M-fish-market"),
    course.urls = c(
      'S-BIOG-061' = !"{assign_url$B06Ia_fish_market}"),
    course.starts = c(
      'S-BIOG-061' = !"{class1_start(mod, 'B06')}"),
    course.ends = c(
      'S-BIOG-061' = !"{n3_end(mod, 'B06')}"),
    term = "Q2", level = 3,
    toc = "Analyse multivarée sur sept poissons (CAH)")
```

## K-moyennes {#kmoyennes}

```{r purlkmeans, echo=FALSE}
#'
#' ### K-moyennes {#kmeans}
#'
```
...
`r h5p(93, "B06Hg_kmeans", height = 270, toc = "Avantages des k-moyennes")`
...
```{r, echo=FALSE, include=TRUE, out.extra='style="border: none;"', out.width='100%', purl=FALSE}
...
`r h5p(94, "B06Hh_kmeans2", height = 270, toc = "Les arguments de la fonction k_means()")`
...
`r h5p(95, "B06Hi_kmeans3", height = 270, toc = "Transformation des données avant k_means()")`
...
```{r assign_B06Ia_fish_market_II, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
...
## Indices de diversité {#indicesdiversite}

```{r purldiversity, echo=FALSE}
#'
...
`r h5p(121, "B06Hj_alpha", height = 270, toc = "Indices de diversité alpha")`
...
`r h5p(122, "B06Hk_jaccard", height = 270, toc = "Indice de Jaccard")`
...
**Créez un projet de groupe par quatre étudiants, que vous continuerez aux modules suivants.**

```{r assign_B06Ga_open_data_I, echo=FALSE, results='asis'}
...
## Récapitulatif des exercices {#recapitulatifb06}
...
-  [Document HTML](more/06-cah-kmeans-div.html)
-  [Script R](https://github.com/BioDataScience-Course/sdd-umons2-2025/blob/main/R/06-cah-kmeans-div.R){target="top"}
