...
`r h5p(46, "B05Ha_base_nls", height = 270, toc = "Bases de la régression non linéaire")`
...
`r h5p(45, "B05Hb_algorithme", height = 270, toc = "Minimisation de la fonction objective")`
...
```{r, echo=FALSE, include=FALSE, eval=FALSE, purl=FALSE}
...
Ensuite, nous recherchons l'aide relative à ces fonctions, par exemple `?SSasymp` (note : il peut y avoir des faux positifs, comme `SSD` ici qui n'a rien à voir avec les fonctions **selfStart**).
...
```{block, type='info', purl=FALSE}
**Reparamétriser un modèle** consiste à exprimer la fonction mathématique qui le représente d'une façon différente. Étant donné que l'interprétation biologique des paramètres fait partie des objectifs de la méthode. On parle d'approche **mécaniste**, qui vise à décrypter le mécanisme sous-jacent *versus* une approche purement **empirique** basée sur les données uniquement avec un modèle polynomial tout venant, par exemple.

Ainsi, L'équation du modèle de von Bertalanffy présentée au début montre une paramétrisation "classique", implémentée dans `SSasympOff()` alors que `SSasymp()` met la taille initiale en évidence au travers du paramètre $R_0$.
```
...
Nous avons ici également représenté les points de manière semi-transparente avec `alpha = 0.2` (transparence de 80%) pour encore mieux mettre en évidence les points de mesures qui se superposent.
...
Notez qu'ici, la couleur a été incluse dans le "mapping" (argument `mapping =`) de `stat_function()` en l'incluant dans `aes()`. Cela change fondamentalement la façon dont la couleur est perçue par {ggplot2}, le moteur graphique utilisé par `chart()`. Dans ce cas-ci, la valeur est interprétée non comme une couleur à proprement parler, mais comme un niveau (une couche) à inclure dans le graphique et à reporter dans une légende. Ensuite, à l'aide de `labs()` on change le titre de la légende relative à la couleur par un nom plus explicite : "Modèle".
...
```{r assign_B05Ia_abies_balsamea, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B05Ia_abies_balsamea", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-015'         = !"{assign_url$B05Ia_abies_balsamea}"),
    course.starts = c(
      'S-BIOG-015'         = !"{class1_start(mod, 'B05')}"),
    course.ends = c(
      'S-BIOG-015'         = !"{n3_end(mod, 'B05')}"),
    term = "Q1", level = 3,
    toc = "Croissance d'un conifère")
```

```{r assign_B02Ga_models_IV, echo=FALSE, results='asis', purl=FALSE}
...
```{r assign_B05Ca_models, echo=FALSE, results='asis', purl=FALSE}
if (exists("challenge"))
  challenge("B05Ca_models", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-015' = !"{assign_url$B05Ca_models}"),
    course.starts = c(
      'S-BIOG-015' = !"{mod['B05', 'challenge']} 10:15:00"),
    course.ends = c(
      'S-BIOG-015' = !"{mod['B05', 'challenge']} 12:30:00"),
...
## Récapitulatif des exercices {#recapitulatifb05}
...
-  [Document HTML](more/05-reg-non-lineaire.html)
-  [Script R](https://github.com/BioDataScience-Course/sdd-umons2-2025/blob/main/R/05-reg-non-lineaire.R){target="top"}
