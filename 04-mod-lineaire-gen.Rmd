...
-   Découvrir le modèle linéaire généralisé (GLM)
...
avec : - $V$ la fonction de variance - $\phi$ (phi) une constante ≥ 0 appelée **paramètre de dispersion** qui permet de contrôler la variance de la distribution de $y$. Ce paramètre de dispersion est fixé à 1 dans certains cas (voir ci-dessous).
...
La GLM binomiale peut être utilisée avec une fonction de lien logit qui ajustera une courbe logistique en forme de S dans ces données, même si cette forme en S est peu marquée semble-t-il ici.
...
La forme en S du modèle est ici bien visible, et elle s'ajuste relativement bien dans les données.

### GLM binomiale avec variable binaire : acariens

```{r purlexbinombin, echo=FALSE}
#'
#' ### GLM binomiale avec variable binaire : acariens
#'
```

Comme troisième exemple, nous allons travailler sur des données de présence/absence (variable binaire, donc qualitative à deux niveaux) de larves d'acariens dans des sols. Les jeux de données `mite` et `mite.env` du package {vegan} répertorient divers acariens dans des sols, ainsi que quelques-unes des propriétés de ces sols. Nous transformons `Oribatl1`, les larves d'acariens *Oribatidae* en présence/absence et conservons `WatrCont` la teneur en eau en g/L ainsi que la topologie du sol (`Topo`, une variable qualitative à deux modalités `Blanket` ou `Hummock`, que vous pouvez interpréter comme terrain plat ou accidenté) comme variables explicatives.
...
Les courbes ne sont pas proches des points. Comment pouvons-nous comprendre cela ? Rappelons-nous que ces courbes représentent, en fait, la **probabilité** d'être "1" (présence) en fonction de la teneur en eau du sédiment sur l'axe X. les points représentent les observations de type présence/absence encodées comme "1" ou "0". Sur l'axe Y, on a la probabilité de présence. Donc, si la courbe prend une valeur Y de 0.75, cela signifie qu'à cette valeur d'humidité sur X, on s'attend à voir les 3/4 des points comme "1", présence et 1/4 des points comme "0", absence. Vous pouvez refaire le même raisonnement tout au long de l'évolution des courbes.
...
`r h5p(237, "B04Hd_formula", height = 270, toc = "Formulation du terme aléatoire")`
...
Nous pouvons calculer l'intervalle de confiance sur ces paramètres avec la fonction `confint()`.
...
`r h5p(238, "B04He_normal", height = 270, toc = "GLMM et normalité des résidus")`
...
La diminution estimée est de 5,6%.
...
```{r assign_B04Ia_lungcap, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B04Ia_lungcap", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-015' = !"{assign_url$B04Ia_lungcap}"),
    course.starts = c(
      'S-BIOG-015' = !"{class1_start(mod, 'B04')}"),
    course.ends = c(
      'S-BIOG-015' = !"{n3_end(mod, 'B04')}"),
    term = "Q1", level = 3,
    toc = "Modèle linéaire généralisé (jeunes fumeurs)")
```

```{r assign_B02Ga_models_III, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment2"))
  assignment2("B02Ga_models", part = "III",
    ...
    course.urls = c(
      'S-BIOG-015' = !"{assign_url$B02Ga_models}"),
    course.starts = c(
      'S-BIOG-015' = !"{class2_start(mod, 'B02')}"),
    course.ends = c(
      'S-BIOG-015' = !"{n4_end(mod, 'B05')}"),
    term = "Q1", level = 4, n = 4,
    toc = "Modélisations libres par quatre (partie III)")
```

## Récapitulatif des exercices {#recapitulatifb04}
...
-  [Document HTML](more/04-mod-lineaire-gen.html)
-  [Script R](https://github.com/BioDataScience-Course/sdd-umons2-2025/blob/main/R/04-mod-lineaire-gen.R){target="top"}
