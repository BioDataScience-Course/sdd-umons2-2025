...
## Outils de diagnostic (suite) {#diagnostic2}
...
### Résumé avec `summary_()`(suite)

Reprenons la sortie renvoyée par `summary_()` appliquée à un objet `lm` (notez ici que nous analysons la sortie de `summary_()` "brute", mais vous savez comment la transformer en un tableau plus présentable dans un rapport avec `tabularise()`).
...
`r h5p(202, "B02Ha_residuals", height = 270, toc = "Résumé lm() et résidus")`
...
`r h5p(203, "B02Hb_predict", height = 270, toc = "Métrique pour modèle prédictif")`


## Régression linéaire multiple {#reglinmultiple}
...
`r h5p(204, "B02Hc_chart", height = 270, toc = "Graphiques d'analyse des résidus")`
...
`r h5p(205, "B02Hd_r_squared", height = 270, toc = "Coefficient de détermination et régression multiple")`
...
summary_(lm(data = trees, volume ~ diameter + diameter2)) |>
  tabularise()
```
...
`r h5p(206, "B02He_poly", height = 270, toc = "Choix de modèle polynomial")`
...
## RMSE & critère d'Akaike {#rmseakaike}
...
`r h5p(207, "B02Hf_rmse", height = 270, toc = "Erreur quadratique moyenne RMSE")`
...
`r h5p(208, "B02Hg_aic", height = 270, toc = "Critère d'akaike")`
...
```{r assign_B02Ia_achatina, echo=FALSE, results='asis', purl=FALSE}
if (exists("assignment"))
  assignment("B02Ia_achatina", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-015' = !"{assign_url$B02Ia_achatina}"),
    course.starts = c(
      'S-BIOG-015' = !"{class1_start(mod, 'B02')}"),
    course.ends = c(
      'S-BIOG-015' = !"{n3_end(mod, 'B02')}"),
    term = "Q1", level = 3,
    toc = "Régression linéaire sur achatina")
```

## Modèle empirique et mécaniste {#modeleempirique}
...
```{r assign_B02Ga_models_I, echo=FALSE, results='asis', purl=FALSE}
...
## Récapitulatif des exercices {#recapitulatifb02}
...
-  [Document HTML](more/02-reg-lineaire-2.html)
-  [Script R](https://github.com/BioDataScience-Course/sdd-umons2-2025/blob/main/R/02-reg-lineaire-2.R){target="top"}
